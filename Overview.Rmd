...
# Fucntion used to recreate ggplot standard colors
...
```{r Tablesummary, echo = FALSE, results=TRUE}
 MySum <- MyDataNumCat %>% 
  group_by(Country) %>% 
  summarize(Q1 = quantile(Poverty, 0.25),
            Mean = mean(Poverty),
            Median = median(Poverty),
            Q3 = quantile(Poverty, 0.75))
...
ggplot(SDGMatPov.Time, aes(x=TimePeriod, y= PovYear)) +
  ...
  geom_point(color = SIAP.color, alpha = 0.5, size = 2)+
...
# create data
set.seed(2512)
time <- as.numeric(rep(seq(from = 2000,to = 2006, by=1), each=7) ) # x Axis
value <- runif(49, 10, 100)               # y Axis
country <- paste("Country", rep(LETTERS[1:7],times=7))# group, one shape per group
MyDataArea <- data.frame(time, value, country)


# stacked area chart
ggplot(MyDataArea, aes(x=time, y=value, fill=country)) + 
