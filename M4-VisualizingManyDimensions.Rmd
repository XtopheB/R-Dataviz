...
# Computing quantiles  of population density
Q_pop=quantile(WV$population_density,c(0,1/4,1/2, 3/4, 1), na.rm = TRUE)
...
WV <- WV %>%
  mutate(
    ...
    Country_density_cat = cut(population_density,  breaks=Q_pop, 
...
WV %>%
 ...
 ggplot() +
 aes(x = CO2_emissions_cat ,fill = region_name, weight = population_density) +
 geom_bar() +
 scale_fill_hue() +
 ggtitle("Population density per CO2 emissions classification of countries (Stacked bars)" ) +
...
ggtitle("Population density per CO2 emissions classification of countries (dodged bars)" ) +
...
WV %>%
 filter(
   ...
   ` Developped` %in%   c("Developing", "Developed") | is.na(` Developped`)) %>%
 ggplot() +
 aes(x = CO2_emissions_cat ,fill = CO2_emissions_cat, weight = population_density) +
 geom_bar(position = "dodge") +
 facet_wrap(~region_name) +
 labs(title ="Population density per CO2 emissions classification of countries \n(Small multiple bars)" ) +
 ylab("Population density")+
 xlab("CO2 emissions category")+
...
# Dual version, looking at CO2 emissions by density categories
