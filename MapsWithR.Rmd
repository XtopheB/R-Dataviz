...
```{r}
# It seems that this exludes many countries
...
asia_pacific_countries <- c(
  ...
  "Cambodia","China","Fiji","India","Indonesia","Iran","Japan","Kazakhstan",
  "Kiribati","Kyrgyzstan","Laos","Malaysia","Maldives","Mongolia","Myanmar",
  "Nepal","Pakistan","Palau","Papua New Guinea","Philippines","Samoa",
  "Singapore","Solomon Islands","South Korea","Sri Lanka","Tajikistan",
  "Thailand","Timor-Leste","Tonga","Turkmenistan","Tuvalu","Uzbekistan",
  "Vanuatu","Vietnam"
)

my_map <- my_sf %>%
  filter(ADMIN %in% asia_pacific_countries)
...
```{r}
# NEW (2025)
# From https://data.worldbank.org/indicator/SG.GEN.PARL.ZS
# Trick: # Do ot read the  description of the file (first lines are skipped)

my_sdg <- read_csv("Data/Maps/API_SG.GEN.PARL.ZS_DS2_en_csv_v2_2495.csv", skip = 3)  
sdg_label <- as.character(my_sdg$`Indicator Name`[1])
```
...
# NEW 
my_geodata <- merge(my_sf_asia, my_sdg, by.x= "NAME_LONG", by.y = "Country Name") %>%
  st_crop(c(xmin= 00, xmax = 180, ymin = -50,  ymax = 70))  # Cropping to get Asia-Pacific

```
```{r}
# Filling missing with previous years. 
my_geodata <-my_geodata  %>%
  mutate(
    X2024 = coalesce(`2024`, `2023`, `2022`, `2021`, `2020`,  `2019`, `2018`, `2017`,  `2016`, `2015`)
  )

# checking (still some missing)
any(is.na(my_geodata$X2024))
...
my_geodata <- bind_rows(my_geodata, vietnam)
```

## with *sf*

```{r}
plot(my_geodata["X2024"], 
     key.pos = 1,
     key.width = lcm(1.3), 
     key.length = 0.5, 
     main = paste(sdg_label, "\n in 2024" ),
     sub = "(SDG indic. 5.5.1)")
```
...
library(ggplot2)
ggplot(my_geodata) + 
  ggtitle(paste(sdg_label, "in 2024"))+
  geom_sf(aes(fill = X2024)) +
   scale_fill_gradient(
...
```{r}
 ggplot(my_geodata) + 
  ...
  theme(legend.position = "bottom", 
        legend.title = element_text(color = "#a50f15", size = 10))
```
...
ggplot(my_geodata) + 
  ...
  geom_sf(aes(fill = X2024)) +
   scale_fill_gradient(
    low = "#fee5d9",   # light red
    high = "#a50f15",  # dark red
    name = "Perc. of women in parliaments \n(SDG. 5.5.1)" ) +
  theme(legend.position = "bottom", 
        legend.title = element_text(color = "#a50f15", size = 10), 
...
```{r}
my_breaks <- quantile(my_geodata$X2024,
...
my_breaks <-  seq(0, max(my_geodata$X2024, na.rm=T),
...
choroLayer(
  ...
  col = rev(heat.colors(my_classes)),
  colNA	= "grey",
  border = "grey", 
  lwd = 0.5,
  legend.pos = c(10,-50), 
...
# Third: add  layout
# layoutLayer(title = paste(sdg_label, "in 2024"),
#             col = "grey",
#             frame = FALSE, 
#             tabtitle = TRUE,
#             posscale =  c(10, -20), 
...
```{r}
# we need to have the lat and longitude of each countries to be able to place the circles (points) 
...
ggplot() + 
 ...
 geom_point(aes(x=my_long, y=my_lat, size= X2024),
            color = SIAP.color ,
            stroke=F, 
            alpha=0.5) +

  ggtitle(sdg_label) + 
  labs(subtitle = "In 2024")+
  labs(fill = "Perc. of women \n(Indic. 5.5.1)")+
  theme_minimal() +
  theme(legend.position = "bottom",
...
```{r}
my_geodata_world %>%
  mutate(area_X2024 = sqrt(X2024))%>%
ggplot() + 
 geom_sf( color = "grey", fill = "white") +
 geom_point(aes(x=my_long, y=my_lat, size= area_X2024),
            color = SIAP.color ,
            alpha=0.5) +
  scale_size_area() +
  ggtitle(sdg_label) + 
...
## Proportional symbols with tmap
...
tm_shape(my_geodata) +
  ...
  tm_bubbles(size = "X2024", scale = 0.9) +
  tm_legend(show = TRUE)
```
...
my_uniform_blue3 =  c('#67a9cf', '#67a9cf', '#67a9cf')
my_uniform_blue3 =  c(SIAP.color, SIAP.color, SIAP.color)
...
```{r}
# Combining plots for IJ IOS
library(cowplot)
library(png)
library(grid)

# Load images
img1 <- rasterGrob(readPNG("Graphics/Choropleth8ClassesN.png"))
img2 <- rasterGrob(readPNG("Graphics/ChoroplethContinuousRedN.png"))
img3 <- rasterGrob(readPNG("Graphics/BubbleMapRedCirclesNew.png"))
img4 <- rasterGrob(readPNG("Graphics/BubbleMapBarsNew.png"))
