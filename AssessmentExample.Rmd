...
Now we can compare the number of deaths due to disasters within each Region and decompose the values according to the each class of GHG emissions. Here again the order of the small multiple graphics has been chosen to reflect the ordering in terms on number of deaths. 
In each region of the world, the number of deaths due to disasters is higher when living a country that is in the "*Low*" class in terms of GHG emissions
...
A simple line plot with each country being colored according to its regions shows already some features. 

- There is an an overall decline in the curves, with maximum values in 2000 declining for 200 to 120.
- The countries in red (Africa) are at the top, while the country at the bottom are in green (Americas) or in blue (Asia).^[Note that we have only data for these 3 regions of the world] 
...
- African countries exhibit the most important decline (almost divided by 2)
...
More globally, we can also consider the evolution over time of the shape of the distributions of this indicator by region and by comparing the distributions between regions of the world. 
...
```{r Ridge, fig.height=6, fig.width=5, out.width = "50%"}
#### Using densities
